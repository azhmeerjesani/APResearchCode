...
from qiskit_aer.noise import NoiseModel, depolarizing_error  # For noise models
...
# Run experiments for all three noise conditions
...
# 2. Single-qubit error runs (1% error per gate)
for i in range(1, n + 1):
...
# 3. Two-qubit error runs (3% error per gate)
for i in range(1, n + 1):
