...
from qiskit_aer.noise import NoiseModel, depolarizing_error, coherent_unitary_error, thermal_relaxation_error
...
def run_qknn_experiment(run_index, noise_type="Noiseless"):
    ...
    if noise_type == "Noisy":
        ...
        # Two-qubit noise operator as tensor product of two Rx rotations
        U = np.kron(Rx, Rx)
        error = coherent_unitary_error(U)
        noise_model = NoiseModel()
        noise_model.add_all_qubit_quantum_error(error, ["cx"])
        ...
        t1 = 1e9  # Set T1 to a very high value so that amplitude damping is negligible.
        gate_time_1q = 50    # Typical single-qubit gate time in ns
        error_1q = thermal_relaxation_error(t1, t2, gate_time_1q)
        noise_model.add_all_qubit_quantum_error(error_1q, ["u1", "u2", "u3"])
        gate_time_2q = 300   # Typical two-qubit gate time in ns
...
# Run experiments for all seven noise conditions
